...
# --- Библиотеки Ursina и дополнительные ---
from ursina import *
from ursina.prefabs.input_field import InputField

# --- Попытка импорта кастомного шейдера прозрачности ---
try:
    from alpha_cube_shader import cube_transparency_shader
except ImportError:
    cube_transparency_shader = None
    print("[WARN] Файл alpha_cube_shader.py не найден. Прозрачность будет отключена.")
...
def resource_path(relative_path):
    """Возвращает абсолютный путь к ресурсам. Работает и при сборке в .exe."""
    try:
...
def register_room_on_master(room_name, game_port=5555):
    """Регистрирует комнату на мастер-сервере."""
    try:
        ...
        print(f"[NET ERROR] Не удалось связаться с мастер-сервером: {e}")


def fetch_rooms_from_master():
    """Получает список активных комнат с мастер-сервера."""
    try:
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.settimeout(2.0)
        s.connect((MASTER_SERVER_IP, MASTER_SERVER_PORT))
        ...
        print(f"[NET ERROR] Ошибка получения списка комнат: {e}")
        return {}


# ==========================================
# 2. ЛОГИЧЕСКОЕ ЯДРО (BACKEND)
# ==========================================
...
class Piece:
    ...
    def is_valid_move(self, target_x, target_y, target_z, board):
        """Проверяет, может ли фигура так ходить (базовая логика)."""
        ...
        if self.name == 'R':  # Ладья – строго по одной оси
            diffs = [target_x != self.x, target_y != self.y, target_z != self.z]
            if sum(diffs) != 1:
                return False
            return True

        # Для прототипа остальные фигуры могут ходить на 1 клетку в любом направлении
...
class NetworkManager:
    ...
    def start_as_host(self, port=5555):
        ...
        self.sock.listen(1)
        print(f"[NET] Сервер запущен на порту {port}. Ожидание подключения...")
    ...
    def connect_to_host(self, ip, port=5555):
        ...
        try:
            ...
            self.conn = self.sock
            print(f"[NET] Подключено к {ip}:{port}. Вы играете за ЧЁРНЫХ.")
    ...
    def _accept_connection(self):
        try:
            ...
            print(f"[NET] Подключён игрок из {addr}. Вы играете за БЕЛЫХ.")
            self._listen_network()
        except Exception as e:
            print(f"[NET] Ошибка приёма соединения: {e}")
    ...
    def send_move(self, fx, fy, fz, tx, ty, tz):
        if self.conn:
            try:
                ...
                print(f"[NET] Ошибка отправки хода: {e}")
    ...
    def send_chat_message(self, nickname, message):
        if self.conn:
            try:
                ...
                print(f"[NET] Ошибка отправки чата: {e}")
    ...
    def _listen_network(self):
        while True:
            try:
                ...
                print("[NET] Связь оборвана! Запуск переподключения...")
    ...
    def start_reconnect_flow(self):
        ...
        self.reconnect_ui = Text(
            text="СВЯЗЬ ПОТЕРЯНА.\nПопытка квантовой ресинхронизации...",
    ...
    def _attempt_reconnect_loop(self):
        ...
        while attempts < max_attempts and self.reconnecting:
            time.sleep(3.0)
            attempts += 1
            print(f"[NET] Попытка реконнекта {attempts}/{max_attempts}...")
            try:
                ...
                if self.is_host:
                    ...
                    self.conn = new_sock
                print("[NET] Соединение восстановлено!")
    ...
    def toggle_chat(self):
        if not self.input_field.enabled:
            ...
            if self.input_field.text.strip():
                my_nick = "Белый Пилот" if self.game.network.my_color == 'white' else "Черный Пилот"
...
class MainMenu(Entity):
    def __init__(self, game_instance):
        ...
        # Панель браузера лобби
        self.lobby_panel = Entity(parent=self, enabled=False)
        self.lobby_list_text = Text(text="Поиск активных секторов в облаке...", scale=1.5, position=(-0.4, 0.1), parent=self.lobby_panel)
    ...
    def set_faction_color(self, selected_color):
        ...
        print(f"[CUSTOM] Цвет фракции: {selected_color}")
    ...
    def open_leaderboard(self):
        ...
        try:
            ...
            self.lead_text.text = "Не удалось загрузить данные с орбитального сервера."
...
class Game3D(Ursina):
    def __init__(self):
        ...
        self.player_custom_color = color.cyan   # выбранный в меню цвет
        ...
        for z in range(8):
            for i in range(8):
                Text3D(
                    ...
                    position=(i, z * 2.0, -1),
                    scale=0.4,
                    color=color.lime if z == 0 else color.gray
                )
                Text3D(
                    text=f"{i+1}:{z}",
                    position=(-1, z * 2.0, i),
    ...
    def render_all_pieces(self):
        ...
        for p in self.board_logic.pieces:
            # Цвет фигуры: свой кастомный для игрока, или стандарт для ИИ/соперника
            if p.color == 'white':
                m_color = self.player_custom_color if (self.ai is None or self.turn == 'white') else color.white
            else:
                m_color = color.black  # чёрные всегда чёрные (можно тоже кастомизировать)

            m_model = 'sphere' if p.name == 'P' else 'cube' if p.name == 'R' else 'diamond'
            visual = Entity(
                model=m_model,
                color=m_color,
    ...
    def on_cell_clicked(self, cell):
        ...
        if self.menu_visible:
            return
        # Если в сетевой игре – ход только своего цвета
        if self.network.conn and self.turn != self.network.my_color:
            print("[NET] Сейчас не ваш ход.")
            return

        cx, cy, cz = cell.coordinates
        clicked_piece = self.board_logic.get_piece_at(cx, cy, cz)

        # Выбор своей фигуры
        if clicked_piece and clicked_piece.color == self.turn:
            self.selected_piece_logic = clicked_piece
            for p, vp in self.visual_pieces.items():
                vp.color = self.player_custom_color if p.color == 'white' else color.black
            self.visual_pieces[clicked_piece].color = color.yellow
            print(f"Выбрана {clicked_piece.name} на X:{cx} Y:{cy} Z:{cz}")
            return

        # Попытка хода
        if self.selected_piece_logic:
            if self.board_logic.is_valid_move(cx, cy, cz, self.board_logic):
                # Запись хода в историю (упрощённо, без файла)
                print(f"Ход: {self.selected_piece_logic.name} -> X:{cx} Y:{cy} Z:{cz}")
                ...
                # Если это сетевая игра – отправляем ход оппоненту
                ...
                # Проверка победы по времени (упрощённо)
                # Проверка на мат (заглушка)

                # Если игра с ИИ – ход ИИ
    ...
    def execute_move(self, piece, tx, ty, tz, is_capture=False):
        ...
        if self.snd_move:
            self.snd_move.play()
        # Лазерный луч (визуальный эффект)
        self.spawn_laser_beam((piece.x, piece.y, piece.z), (tx, ty, tz), color.cyan if self.turn == 'white' else color.magenta)

        # Проверка на квантовый взрыв пешки
        if piece.name == 'P':
    ...
    def execute_network_move(self, fx, fy, fz, tx, ty, tz):
        """Выполнение хода, полученного по сети."""
        piece = self.board_logic.get_piece_at(fx, fy, fz)
        if piece:
            # Просто применяем ход без проверки (считаем, что соперник его уже проверил)
            self.board_logic.move_piece(piece, tx, ty, tz)
            self.render_all_pieces()
            self.turn = 'black' if self.turn == 'white' else 'white'
            if self.snd_move:
                self.snd_move.play()
            # Если после хода оппонента ход переходит к игроку, ничего не делаем (ждём его клик)

    # ---------- ЛАЗЕРНЫЙ ЛУЧ ----------
    def spawn_laser_beam(self, start, end, beam_color=color.cyan):
        p1 = Vec3(start[0], start[2] * 2.0, start[1])
        p2 = Vec3(end[0], end[2] * 2.0, end[1])
    ...
    def trigger_quantum_explosion(self, cx, cy, cz):
        # Визуальный взрыв (упрощённо)
        explosion_center = Vec3(cx, cz * 2.0, cy)
        wave = Entity(
            model='sphere',
            color=color.rgba(255, 69, 0, 180),
            position=explosion_center,
            scale=0.1
        )
        wave.animate_scale(Vec3(3.0, 6.0, 3.0), duration=0.4, curve=curve.out_expo)
        ...
        for _ in range(25):
            particle = Entity(
                ...
                position=explosion_center + Vec3(random.uniform(-0.2, 0.2), random.uniform(-0.2, 0.2), random.uniform(-0.2, 0.2)),
    ...
    def ai_turn(self):
        if self.ai and self.turn == self.ai.ai_color and self.game_active:
            ...
            if piece:
                self.execute_move(piece, tx, ty, tz, is_capture=False)
                self.turn = 'black' if self.turn == 'white' else 'white'
                self.render_all_pieces()
                # Если после хода ИИ снова его ход (не должно быть), но на всякий случай
    ...
    def end_game_by_timeout(self, loser_color):
        ...
        # Отправка на мастер-сервер (заглушка)
    ...
    def start_singleplayer_game(self):
        ...
        self.time_white = 180.0
        self.time_black = 180.0
        print("[GAME] Одиночная игра против ИИ начата.")

    # ---------- ОБРАБОТКА ВВОДА КЛАВИАТУРЫ ----------
    def input(self, key):
        if key == 'enter':
            self.chat_ui.toggle_chat()
        # Можно добавить другие клавиши (например, Esc для меню)


# ==========================================
# 8. МОДУЛЬ ТЕСТИРОВАНИЯ (QA)
# ==========================================
...
def run_backend_tests():
    print("\n[QA] Запуск автоматических юнит-тестов...")
    suite = unittest.TestLoader().loadTestsFromTestCase(TestSpheresOfPower)
    runner = unittest.TextTestRunner(verbosity=2)
    result = runner.run(suite)
    if result.wasSuccessful():
        print("[QA] Все тесты пройдены успешно!")
    else:
        print("[QA] КРИТИЧЕСКАЯ ОШИБКА: Тесты не пройдены.")
